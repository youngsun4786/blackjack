 ...
 class BlackJackGame{
     public:
        ...
        BlackJackGame(){
            ...
            if (typeid(numPlayers) != typeid(int())) {
...
void BlackJackGame::play(){
    ...
    // deal two cards to each of the player
    for (int i = 0; i < 2; i++) {
        //iterate through the list of players
        for (int j = 0; j < m_player.size(); j++){
             m_deck.deal(m_player[i].h);
        }
    }
    ...
    std:: cout << "[" << m_casino.h.getTotal() << "]" << endl;
    // print the player's information
    for (int i = 1; i <= m_player.size(); i++) {
        std::cout << "Player " << i << ": ";
        m_player[i].h.printDeck();
        std:: cout << "[" << m_player[i].h.getTotal() << "]" << endl;
    }

    
    // // while player has not busted
    // while (!m_player.isBusted(m_player.h)) { 
    ...
    //         std:: cout << "[" << m_player.h.getTotal() << "]" << endl;
    //     } else {
    //         break;
    //     }
    // }
    ...
    // if(!m_player.isBusted(m_player.h)) {
    //     while (!m_casino.isBusted(m_casino.h)) {
    ...
    //             if (m_casino.getWinRate() < 45 && m_casino.h.getTotal() > 10) {
    //                 Card Blackjack;
    //                 for (int i = 0; i < m_deck.cards.size(); i++) {
    //                     if( m_casino.h.getTotal() + m_deck.cards[i].getValue() <= 21) {
    //                         Blackjack = m_deck.cards[i];
    //                         m_deck.remove(Blackjack);
    //                         m_casino.h.add(Blackjack);
    //                         break;
    //                     }
    //                 }
...
int main() {
 ...
 // every time you bust increment the win for the dealer
 // if to calculate the win ratio you must sum the wins from the hand 
